/**
 *    Candeo C-ZB-SEMO Zigbee Motion Sensor
 *    Reports Motion Events
 *    Reports Illuminance Events
...
metadata {
    definition(name: 'Candeo C-ZB-SEMO Zigbee Motion Sensor', namespace: 'Candeo', author: 'Candeo', importUrl: '', singleThreaded: true) {
        ...
        fingerprint profileId: '0104', endpointId: '01', inClusters: '0000,0003,0500,0001,0400', manufacturer: 'Candeo', model: 'C-ZB-SEMO', deviceJoinName: 'Candeo C-ZB-SEMO Zigbee Motion Sensor'
    }
    preferences {
...
private @Field final String CANDEO = 'Candeo C-ZB-SEMO Device Driver'
...
List<String> configure() {
    ...
    List<String> cmds = ["zdo bind 0x${device.deviceNetworkId} 0x${device.endpointId} 0x01 0x0001 {${device.zigbeeId}} {}", "delay ${ZIGBEEDELAY}",
                    ...
                    "zdo bind 0x${device.deviceNetworkId} 0x${device.endpointId} 0x01 0x0400 {${device.zigbeeId}} {}", "delay ${ZIGBEEDELAY}",
                    "he cr 0x${device.deviceNetworkId} 0x${device.endpointId} 0x0400 0x0000 0x21 0 65535 {0} {}", "delay ${ZIGBEEDELAY}",
                    "he raw 0x${device.deviceNetworkId} 0x01 0x${device.endpointId} 0x0400 {10 00 08 00 0000}", "delay ${ZIGBEEDELAY}",
                    "he rattr 0x${device.deviceNetworkId} 0x${device.endpointId} 0x0400 0x0000 {}"]
...
private List<Map> processIASEvents(String description, List<Map> events = []) {
    ...
    if (description.startsWith('zone status')) {
        ...
        if (zoneStatus != null) {
            ...
            String motionState = alarmState ? 'active' : 'inactive'
            String descriptionText = "${device.displayName} motion is ${motionState}"
            logEvent(descriptionText)
            events.add(processEvent([name: 'motion', value: motionState, descriptionText: descriptionText]))
        }
        else {
...
private List<Map> processEvents(Map descriptionMap, List<Map> events = []) {
    ...
    if (descriptionMap.profileId && descriptionMap.profileId == '0000') {
        ...
        else if (descriptionMap.cluster == '0400' || descriptionMap.clusterId == '0400' || descriptionMap.clusterInt == 1024) {
            processIlluminanceMeasurementCluster(descriptionMap, events)
        }
        else {
...
private void processIlluminanceMeasurementCluster(Map descriptionMap, List<Map> events) {
    logTrace('processIlluminanceMeasurementCluster called')
    switch (descriptionMap.command) {
        case '0A':
        case '01':
            if (descriptionMap.attrId == '0000' || descriptionMap.attrInt == 0) {
                logDebug('illuminance measurement (0400) measured value report (0000)')
                Integer illuminanceValue = zigbee.convertHexToInt(descriptionMap.value)
                ...
                logDebug('illuminance measurement (0400) attribute skipped')
            }
            break

        case '04':
            logDebug('illuminance measurement (0400) write attribute response (04) skipped')
            break
        case '07':
            logDebug('illuminance measurement (0400) configure reporting response (07) skipped')
            break
        case '0B':
            logDebug('illuminance measurement (0400) default response (0B) skipped')
            break
        default:
            logDebug('illuminance measurement (0400) command skipped')
            break
    }
}
