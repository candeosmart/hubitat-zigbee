/**
 *    Candeo C-ZB-SM30-2G Zigbee 2-Gang Switch Module (L+N)
...
metadata {
    definition(name: 'Candeo C-ZB-SM30-2G Zigbee 2-Gang Switch Module (L+N)', namespace: 'Candeo', author: 'Candeo', importUrl: 'https://raw.githubusercontent.com/candeosmart/hubitat-zigbee/refs/heads/main/Candeo%20C-ZB-SM30-2G%20Zigbee%202-Gang%20Switch%20Module%20(L%2BN).groovy', singleThreaded: true) {
        capability 'Switch'
        capability 'Flash'
        ...
        command 'onWithAutomaticOff', [ [ name:'Automatic Off Time*', type: 'NUMBER', description: 'Automatically turn the output off after this time (ms).' ] ]
        command 'resetPreferencesToDefault'
        command 'resetAllAlarms'

        fingerprint profileId: '0104', endpointId: '01', inClusters: '0000,0003,0004,0005,0006,1000', manufacturer: 'Candeo', model: 'C-ZB-SM30-2G', deviceJoinName: 'Candeo C-ZB-SM30-2G Zigbee 2-Gang Switch Module (L+N)'
    }
    preferences {
...
private @Field final String CANDEO = 'Candeo C-ZB-SM30-2G Device Driver'
...
private @Field final String PARENTEP = '03'
...
void resetAllAlarms() {
    logTrace('resetAllAlarms called')
    List<String> cmds = ["he cmd 0x${device.deviceNetworkId} 0x${PARENTEP} 0x0009 0x01 {}", "delay ${ZIGBEEDELAY}"]
    doZigBeeCommand(cmds)
}

void onWithAutomaticOff(BigDecimal automaticOffTime) {
    logTrace("onWithAutomaticOff called automaticOffTime: ${automaticOffTime}")
    CHILDEPS.each { String childEP ->
        String address = "EP${childEP}"
        ChildDeviceWrapper childDevice = findChildDevice(address)
        if (childDevice) {
            logDebug("got child device name: ${childDevice.name} displayName: ${childDevice.displayName}, sending on")
            childDevice.onWithAutomaticOff(automaticOffTime)
        }
        else {
            logWarn('could not find child device, skipping command!')
        }
...
void componentOn(DeviceWrapper childDevice, Integer onTime) {
    logTrace("componentOn called onTime: ${onTime}")
    logDebug("got childDevice: ${childDevice.displayName}")
    String endpoint = childDevice.deviceNetworkId.split('-EP')[1]
    List<String> cmds = ["he cmd 0x${device.deviceNetworkId} 0x${endpoint} 0x0006 0x42 {00 ${convertToHexString(onTime, 2, true)} 00 00 }", "delay ${ZIGBEEDELAY}"]
    doZigBeeCommand(cmds)
}
...
void componentConfigure(DeviceWrapper childDevice) {
    ...
    Map<String, Integer> startUpOnOff = ['on': 1, 'off': 0, 'previous': 255]
    ...
    List<String> cmds = [//onoff
                         ...
                         "he wattr 0x${device.deviceNetworkId} 0x${endpoint} 0x0006 0x4003 ${convertToHexString(DataType.ENUM8)} {${convertToHexString(startUpOnOff[deviceConfigDefaultPowerOnBehaviour ?: 'previous'])}} {}", "delay ${ZIGBEEDELAY}",
                         "he rattr 0x${device.deviceNetworkId} 0x${endpoint} 0x0006 0x4003 {}", "delay ${ZIGBEEDELAY}",
                         ]
    doZigBeeCommand(cmds)
}
...
private List<Map> processEvents(Map descriptionMap, List<Map> events = []) {
    ...
    if (descriptionMap.profileId && descriptionMap.profileId == '0000') {
        ...
        else if (endpoint == PARENTEP && (descriptionMap.cluster == '0009' || descriptionMap.clusterId == '0009' || descriptionMap.clusterInt == 9)) {
            processAlarmCommand(descriptionMap, events)
        }
...
private void processAlarmCommand(Map descriptionMap, List<Map> events) {
    logTrace('processAlarmCommand called')
    switch (descriptionMap.command) {
        case '00':
            logDebug('alarm (0009) alarm command')
            String alarmCommand = descriptionMap.data[0]
            logDebug("alarm command is ${alarmCommand}")
            String clusterIdentifier = descriptionMap.data[1..2]
            logDebug("cluster identifier is ${clusterIdentifier}")
            break
        default:
            logDebug('alarm (0009) command skipped')
            break
    }
}
...
private ChildDeviceWrapper findChildDevice(String address) {
    ...
    if (childDevice == null) {
        ...
        this.addChildDevice('Candeo', 'Candeo C-ZB-SM30-2G Zigbee 2-Gang Switch Module (L+N) Child Switch', "${device.id}-${address}", [name: "${device.displayName} ${address}", label: "${device.displayName} ${address}", isComponent: true])
