/**
 *    Candeo C-ZB-LC20 Smart LED Controller (RGBCCT Mode)
 *    Supports Momentary Switches
 *    Supports on / off / setLevel / startLevelChange / stopLevelChange / setColorTemperature / setColor / setHue / setSaturation / flash
 *    Reports switch / level / rgb / color / colorName / hue / saturation / colorName / colorTemperature / colorMode events
 *    Has Setting For Level Transition Time
 *    Has Setting For Level Change Time
 *    Has Setting For Colour Transition Time
 ...
 *    RGBCCT Mode (WCBGRV+) - In this mode the device uses the W and C channels to present CCT values.  It then uses the BGR channels to present full colours.
 */

metadata {
    definition(name: 'Candeo C-ZB-LC20 Smart LED Controller (RGBCCT Mode)', namespace: 'Candeo', author: 'Candeo', importUrl: 'https://raw.githubusercontent.com/candeosmart/hubitat-zigbee/refs/heads/main/Candeo%20C-ZB-LC20%20Smart%20LED%20Controller%20(RGBCCT%20Mode).groovy', singleThreaded: true) {
        capability 'Switch'
        ...
        fingerprint profileId: '0104', endpointId: '0B', inClusters: '0000,0003,0004,0005,0006,0008,0300,1000', outClusters: '0019', manufacturer: 'Candeo', model: 'C-ZB-LC20-RGBCCT', deviceJoinName: 'Candeo C-ZB-LC20 Zigbee Smart Controller For LED Strips (RGBCCT Mode)'
        fingerprint profileId: '0104', endpointId: '0B', inClusters: '0000,0003,0004,0005,0006,0008,0300,1000', outClusters: '0019', manufacturer: 'Candeo', model: 'C-ZB-LC20v2-RGBCCT', deviceJoinName: 'Candeo C-ZB-LC20 Zigbee Smart Controller For LED Strips (RGBCCT Mode)'
    }
    preferences {
        ...
        input name: 'colourTransitionTime', type: 'enum', title: 'Colour Transition Time (s)', description: '<small>When setting the device to a specific colour, use this as the default transition time if one is not specified explicitly.</small><br><br>', options: PREFTRANSITIONTIME, defaultValue: '1000'
...
private @Field final Map COLOURNAMES = [ 15: 'Red', 45: 'Orange', 75: 'Yellow', 105: 'Chartreuse', 135: 'Green', 165: 'Spring', 195: 'Cyan', 225: 'Azure', 255: 'Blue', 285: 'Violet', 315: 'Magenta', 345: 'Rose', 361: 'Red' ]
...
void installed() {
    ...
    device.updateSetting('colourTransitionTime', [value: '1000', type: 'enum'])
    logInfo("colourTransitionTime setting is: ${PREFTRANSITIONTIME[colourTransitionTime]}")
...
void updated() {
    ...
    logInfo("colourTransitionTime setting is: ${PREFTRANSITIONTIME[colourTransitionTime ?: '1000']}", true)
...
List<String> setColorValues(BigDecimal hue, BigDecimal saturation, BigDecimal level) {
    logTrace("setColorValues called hue: ${hue} saturation: ${saturation} level: ${level}")
    Map<String,BigDecimal> colorMap = ['hue': hue, 'saturation': saturation, 'level': level]
    return setColor(colorMap, null)
}

List<String> setColorValues(BigDecimal hue, BigDecimal saturation, BigDecimal level, BigDecimal rate) {
    logTrace("setColorValues called hue: ${hue} saturation: ${saturation} level: ${level} rate: ${rate}")
    Map<String,BigDecimal> colorMap = ['hue': hue, 'saturation': saturation, 'level': level]
    return setColor(colorMap, rate)
}

List<String> setColor(Map<String,BigDecimal> colorMap) {
    logTrace("setColor called colorMap: ${colorMap}")
    return setColor(colorMap, null)
}

List<String> setColor(Map<String,BigDecimal> colorMap, BigDecimal rate) {
    logTrace("setColor called colorMap: ${colorMap}")
    Integer hue = colorMap.hue.toInteger()
    Integer saturation = colorMap.saturation.toInteger()
    Integer level = colorMap.level.toInteger()
    logTrace("hue: ${hue} saturation: ${saturation} level: ${level}")
    List<String> cmds = []
    Integer scaledRate = null
    if (rate != null) {
        scaledRate = (rate * 10).toInteger()
    ...
    Integer scaledLevel = (level * 2.55).toInteger()
    if (level > 100 || level < 0) {
        scaledLevel = ((level > 100 ? 100 : level < 0 ? 0 : level) * 2.54).toInteger()
        logWarn('level outside range supported by device (0 - 100), resetting to closest value!')
    }
    logDebug("scaledLevel: ${scaledLevel}")
    cmds += ["he cmd 0x${device.deviceNetworkId} 0x${device.endpointId} 0x0008 4 {0x${intTo8bitUnsignedHex(scaledLevel)} 0x${intTo16bitUnsignedHex(scaledRate)}}", "delay ${ZIGBEEDELAY}"]
    if (scaledRate == null) {
        logDebug("colourTransitionTime: ${colourTransitionTime ?: '1000'}")
        BigDecimal colourTime = colourTransitionTime ? colourTransitionTime.toBigDecimal() : 1000
        logDebug("colourTime: ${colourTime ?: '1000'}")
        scaledRate = (colourTime / 1000 * 10).toInteger()
        logDebug("scaledRate: ${scaledRate}")
    }
    Integer scaledSaturation = (saturation * 2.54).toInteger()
    if (saturation > 100 || saturation < 0) {
        scaledSaturation = ((saturation > 100 ? 100 : saturation < 0 ? 0 : saturation) * 2.54).toInteger()
        logWarn('saturation outside range supported by device (0 - 100), resetting to closest value!')
    }
    logDebug("scaledSaturation: ${scaledSaturation}")
    Integer scaledHue = (hue * 2.54).toInteger()
    if (hue > 100 || hue < 0) {
        scaledHue = ((hue > 100 ? 100 : hue < 0 ? 0 : hue) * 2.54).toInteger()
...
List<String> setHue(BigDecimal hue) {
    logTrace("setHue called hue: ${hue}")
    return setHue(hue, null)
}

List<String> setHue(BigDecimal hue, BigDecimal rate) {
    logTrace("setHue called hue: ${hue}")
    List<String> cmds = []
    Integer scaledRate = null
    if (rate != null) {
...
List<String> setSaturation(BigDecimal saturation) {
    logTrace("setSaturation called saturation: ${saturation}")
    return setSaturation(saturation, null)
}

List<String> setSaturation(BigDecimal saturation, BigDecimal rate) {
    logTrace("setSaturation called saturation: ${saturation}")
    List<String> cmds = []
    Integer scaledRate = null
    if (rate != null) {
    ...
    switch (descriptionMap.command) {
        ...
        case '01':
            if (descriptionMap.attrId == '0000' || descriptionMap.attrInt == 0) {
                ...
                String colourMode = device.currentValue('colorMode') ?: 'unknown'
                if (colourMode == 'RGB') {
                    Integer currentHue = device.currentValue('hue') ? (device.currentValue('hue')).toInteger() : -1
                    Integer currentSaturation = device.currentValue('saturation') ? (device.currentValue('saturation')).toInteger() : -1
                    Integer currentLevel = device.currentValue('level') ? (device.currentValue('level')).toInteger() : -1
                    if (currentHue != -1 && currentSaturation != -1 && currentLevel != -1) {
                        events.add(processEvent([name: 'color', value: [ hue: currentHue, saturation: currentSaturation, level: currentLevel ] ]))
                    }
                    List<Integer> rgb = hubitat.helper.ColorUtils.hsvToRGB([currentHue, currentSaturation, currentLevel])
                    events.add(processEvent([name: 'RGB', value: [ rgb[0], rgb[1], rgb[2] ] ]))
                }
            }
...
private void processColourEvent(Map descriptionMap, List<Map> events) {
    ...
    switch (descriptionMap.command) {
        ...
        case '01':
            if (descriptionMap.attrId == '0000' || descriptionMap.attrInt == 0) {
                logDebug('colour control (0300) current hue report (0000)')
                Integer hueValue = zigbee.convertHexToInt(descriptionMap.value)
                logDebug("current hue report is ${hueValue}")
                hueValue = hueValue <= 254 ? hueValue >= 0 ? hueValue : 0 : 254
                hueValue = Math.round(hueValue / 2.54)
                String descriptionText = "${device.displayName} was set to ${hueValue}%"
                String currentValue =  device.currentValue('hue') ?: 'unknown'
                if (hueValue == currentValue) {
                    descriptionText = "${device.displayName} is ${hueValue}%"
                }
                logEvent(descriptionText)
                events.add(processEvent([name: 'hue', value: hueValue, unit: '%', type: type, descriptionText: descriptionText]))
                String colourName = COLOURNAMES.find { k , v -> Math.round(hueValue * 3.6) < k }.value
                logDebug("colourName is ${colourName}")
                descriptionText = "${device.displayName} is ${colourName}"
                logEvent(descriptionText)
                events.add(processEvent([name: 'colorName', value: colourName, descriptionText: descriptionText]))
            }
            else if (descriptionMap.attrId == '0001' || descriptionMap.attrInt == 1) {
                logDebug('colour control (0300) current saturation report (0000)')
                Integer saturationValue = zigbee.convertHexToInt(descriptionMap.value)
                logDebug("current saturation report is ${saturationValue}")
                saturationValue = saturationValue <= 254 ? saturationValue >= 0 ? saturationValue : 0 : 254
                saturationValue = Math.round(saturationValue / 2.54)
                String descriptionText = "${device.displayName} was set to ${saturationValue}%"
                String currentValue =  device.currentValue('saturation') ?: 'unknown'
                if (saturationValue == currentValue) {
                    descriptionText = "${device.displayName} is ${saturationValue}%"
                }
                logEvent(descriptionText)
                events.add(processEvent([name: 'saturation', value: saturationValue, unit: '%', type: type, descriptionText: descriptionText]))
            }
            else if (descriptionMap.attrId == '0007' || descriptionMap.attrInt == 7) {
