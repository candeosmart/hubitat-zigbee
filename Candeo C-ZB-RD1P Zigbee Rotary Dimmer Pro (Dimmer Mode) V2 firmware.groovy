/**
 *    Candeo C-ZB-RD1P Zigbee Rotary Dimmer Pro (Dimmer Mode)
 *    Supports on / off / setLevel / startLevelChange / stopLevelChange / flash
 *    Reports switch / level / power / energy / current / voltage events
 *    Reports button 1 double tapped, held and released events for double press / hold / release on knob
...
metadata {
    definition(name: 'Candeo C-ZB-RD1P Zigbee Rotary Dimmer Pro (Dimmer Mode) V2 firmware', namespace: 'Candeo', author: 'Candeo', importUrl: 'https://raw.githubusercontent.com/candeosmart/hubitat-zigbee/refs/heads/main/Candeo%20C-ZB-RD1P%20Zigbee%20Rotary%20Dimmer%20Pro%20(Dimmer%20Mode)%20V2%20firmware.groovy', singleThreaded: true) {
        capability 'Switch'
        ...
        fingerprint profileId: '0104', endpointId: '01', inClusters: '0000,0003,0004,0005,0006,0008,0702,0B04,1000', outClusters: '0003,0019', manufacturer: 'Candeo', model: 'C-ZB-RD1Pv2-DIM', deviceJoinName: 'Candeo C-ZB-RD1P Zigbee Rotary Dimmer Pro (Dimmer Mode)'
    }
    preferences {
        ...
        input name: 'levelChangeRate', type: 'enum', title: 'Level Change Rate (%)', description: '<small>When carrying out a start level change command, move at this percentage per second.</small><br><br>', options: PREFLEVELCHANGERATE, defaultValue: 'device'
        ...
        input name: 'deviceConfigDefaultOnTransitionTime', type: 'enum', title: 'On Transition Time (s) When Turned On', description: '<small>When turned on, use this as the transition time to fade up.</small><br><br>', options: PREFTRANSITIONTIME, defaultValue: '1000' //10ths of a second default 10
        input name: 'deviceConfigDefaultOffTransitionTime', type: 'enum', title: 'Off Transition Time (s) When Turned Off', description: '<small>When turned off, use this as the transition time to fade down.</small><br><br>', options: PREFTRANSITIONTIME, defaultValue: '1000' //10ths of a second default 10
        input name: 'deviceConfigDefaultMoveRate', type: 'enum', title: 'Move Rate Per Second (%)', description: '<small>When changing level, move at this percentage per second.</small><br><br>', options: PREFMOVERATE, defaultValue: '35' //level units per second default 85, approximately 35%
...
void installed() {
    ...
    sendEvent(processEvent(name: 'numberOfButtons', value: 1, displayed: false))
    sendEvent(buttonAction('doubleTapped', 1, 'digital'))
}
...
List<String> configure() {
    ...
    Map<String, Integer> startUpOnOff = ['on': 1, 'off': 0, 'opposite': 2, 'previous': 255]
    ...
    switch (descriptionMap.command) {
        ...
        case '01':
            if (descriptionMap.attrId == '0000' || descriptionMap.attrInt == 0) {
                ...
                logDebug("deviceConfigDefaultOnTransitionTime is currently set to: ${PREFTRANSITIONTIME[deviceConfigDefaultOnTransitionTime ?: '1000']} and device reports it is set to: ${PREFTRANSITIONTIME[(onTransitionTimeValue * 100).toString()]}")
            }
                ...
                logDebug("deviceConfigDefaultOffTransitionTime is currently set to: ${PREFTRANSITIONTIME[deviceConfigDefaultOffTransitionTime ?: '1000']} and device reports it is set to: ${PREFTRANSITIONTIME[(offTransitionTimeValue * 100).toString()]}")
            }
...
private void setPreferencesToDefault() {
    ...
    device.updateSetting('deviceConfigDefaultOnTransitionTime', PREF1000)
    logInfo("deviceConfigDefaultOnTransitionTime setting is: ${PREFTRANSITIONTIME[deviceConfigDefaultOnTransitionTime]}")
    device.updateSetting('deviceConfigDefaultOffTransitionTime', PREF1000)
    logInfo("deviceConfigDefaultOffTransitionTime setting is: ${PREFTRANSITIONTIME[deviceConfigDefaultOffTransitionTime]}")
    device.updateSetting('deviceConfigDefaultMoveRate', [value: '35', type: 'enum'])
    logInfo("deviceConfigDefaultMoveRate setting is: ${PREFMOVERATE[deviceConfigDefaultMoveRate]}")
    ...
    device.updateSetting('levelChangeRate', PREFDEVICE)
