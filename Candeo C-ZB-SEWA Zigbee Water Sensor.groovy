/**
 *    Candeo C-ZB-SEWA Zigbee Water Sensor
 *    Reports Water Events
...
metadata {
    definition(name: 'Candeo C-ZB-SEWA Zigbee Water Sensor', namespace: 'Candeo', author: 'Candeo', importUrl: '', singleThreaded: true) {
        ...
        fingerprint profileId: '0104', endpointId: '01', inClusters: '0000,0003,0500,0001', outClusters: '000A', manufacturer: 'Candeo', model: 'C-ZB-SEWA', deviceJoinName: 'Candeo C-ZB-SEWA Zigbee Water Sensor'
    }
    preferences {
...
private @Field final String CANDEO = 'Candeo C-ZB-SEWA Device Driver'
...
private List<Map> processIASEvents(String description, List<Map> events = []) {
    ...
    if (description.startsWith('zone status')) {
        ...
        if (zoneStatus != null) {
            ...
            String waterState = alarmState ? 'wet' : 'dry'
            String descriptionText = "${device.displayName} water is ${waterState}"
            logEvent(descriptionText)
            events.add(processEvent([name: 'water', value: waterState, descriptionText: descriptionText]))
        }
        else {
