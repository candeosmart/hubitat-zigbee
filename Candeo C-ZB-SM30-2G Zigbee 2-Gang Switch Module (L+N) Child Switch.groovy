/**
 *    Candeo C-ZB-SM30-2G Zigbee 2-Gang Switch Module (L+N) Child Switch
 ...
 *    Has Setting For Automatic Off Time
...
metadata {
    ...
    preferences {
        ...
        input name: 'automaticOffTime', type: 'enum', title: 'Automatic Off Time (s)', description: '<small>When turning the device on, use this as the automatic off time if one is not specified explicitly.</small><br><br>', options: PREFAUTOMATICOFFTIME, defaultValue: 'none'
...
private @Field final Map PREFPOWERON = [ 'off': 'Off', 'on': 'On', 'previous': 'Previous' ]
private @Field final Map PREFHUBRESTART = [ 'off': 'Off', 'on': 'On', 'refresh': 'Refresh State Only', 'nothing': 'Do Nothing' ]
private @Field final Map PREFAUTOMATICOFFTIME = ['none': 'do not automatically turn off the output', '500': '0.5s', '1000': '1s', '1500': '1.5s', '2000': '2s', '2500': '2.5s', '3000': '3s', '3500': '3.5s', '4000': '4s', '4500': '4.5s', '5000': '5s', '10000': '10s', '20000': '20s', '30000': '30s', '40000': '40s', '50000': '50s', '60000': '60s', '90000': '90s', '120000': '120s', '240000': '240s', '300000': '300s', '600000': '600s', '900000': '900s', '1800000': '1800s', '3600000': '3600s']
...
void resetPreferencesToDefault() {
    ...
    device.updateSetting('automaticOffTime', PREFNONE)
    logInfo("automaticOffTime setting is: ${PREFAUTOMATICOFFTIME[automaticOffTime]}")
...
void updated() {
    ...
    logInfo("automaticOffTime setting is: ${PREFAUTOMATICOFFTIME[automaticOffTime ?: 'none']}", true)
...
void onWithAutomaticOff(BigDecimal automaticOffTime) {
    ...
    flashStop(false)
    Integer onTime = (automaticOffTime.toInteger() / 100).toInteger()
    logDebug("onTime: ${onTime}")
    if (onTime != 65535) {
        parent?.componentOn(this.device, onTime)
    }
    else {
...
void on() {
    logTrace('on called')
    flashStop(false)
    logDebug("automatic off time is ${automaticOffTime ?: 'none'}")
    Integer onTime = automaticOffTime ? automaticOffTime == 'none' ? 65535 : (automaticOffTime.toInteger() / 100).toInteger() : 65535
    logDebug("onTime: ${onTime}")
    if (onTime != 65535) {
        parent?.componentOn(this.device, onTime)
    }
